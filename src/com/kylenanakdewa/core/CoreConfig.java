...
public final class CoreConfig {
    ...
    public static final boolean searchDisplayNames = config.getConfig().getBoolean("general.search-player-displaynames");
    ...
    public static final List<String> realmLoadSource = config.getConfig().getStringList("core.load-realms-from");

    //// Scoreboard
    // Whether scoreboard integration is enabled - default TRUE
    public static final boolean useScoreboard = config.getConfig().getBoolean("core.use-minecraft-scoreboard");

    // Whether friendly-fire is blocked - default FALSE
    public static final boolean blockFriendlyFire = config.getConfig().getBoolean("core.scoreboard-options.block-friendly-fire");
    // Whether realm members see friendly invisibles - default FALSE
    public static final boolean seeFriendlyInvisibles = config.getConfig().getBoolean("core.scoreboard-options.see-friendly-invisibles");
    // Whether name tags are visible - default "always"
    public static final String nameTagVisibility = config.getConfig().getString("core.scoreboard-options.name-tag-visibility");
    // Whether death messages are visible - default "always"
    public static final String deathMessageVisibility = config.getConfig().getString("core.scoreboard-options.death-message-visibility");
    // Whether players collide - default "always"
    public static final String collisionRule = config.getConfig().getString("core.scoreboard-options.collision-rule");

    //// Creation
    // Players needed to start a realm - default 1
    public static final int playersForNewRealm = config.getConfig().getInt("core.players-for-new-realm");
    // Whether players can create new realms, instead of just sub-realms - default TRUE
    public static final boolean playersCreateNewRealm = config.getConfig().getBoolean("core.players-create-new-realms");
    // Whether only officers can create new sub-realms - default FALSE
    public static final boolean officersCreateSubRealms = config.getConfig().getBoolean("core.officers-create-sub-realms");
}