...
public final class PermissionsCommands implements TabExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(!CoreConfig.permsEnabled){
			...
			sender.sendMessage(CommonColors.ERROR+"RealmsCore permissions are not enabled on this server. Use another plugin to manage permissions, or enable permissions in the RealmsCore config. See http://wolfiamc.com/plugins/realmscore for help.");
			return true;
		}
		...
		if(args[0].equalsIgnoreCase("set")){
			...
			if(args.length==3 && args[1].equalsIgnoreCase("info")){
				if(!sender.hasPermission("core.permissions.set")) return Error.NO_PERMISSION.displayChat(sender);
			...
			if(args.length==3 && sender.hasPermission("core.permissions.setswitch.others")){
				...
				if(!PermsUtils.isDoubleCheckedAdmin(sender)){
					Utils.notifyAdminsError(sender.getName()+CommonColors.ERROR+" failed security check (command: /permissions "+String.join(" ", args)+").");
				...
				if(targetPlayer.equals(sender)){
					sender.sendMessage(CommonColors.ERROR+"You must use this command to switch your own permissions: /set <set name>");
		...
		if(args.length>=2 && args[0].equalsIgnoreCase("player")){
			...
			if(args.length==4 && args[2].equalsIgnoreCase("set")){
				...
				if(!sender.hasPermission("core.permissions.setswitch.others")){
					Utils.notifyAdminsError(sender.getName()+CommonColors.ERROR+" failed security check (switching "+targetPlayer.getName()+" to "+args[3]+" permissions).");
					return Error.NO_PERMISSION.displayChat(sender);
				}
				if(targetPlayer.equals(sender)){
					sender.sendMessage(CommonColors.ERROR+"You must use this command to switch your own permissions: /set <set name>");
			...
			if(args.length==3 && args[2].equalsIgnoreCase("revoke")){
				...
				if(!sender.hasPermission("core.permissions.revoke")){
					Utils.notifyAdminsError(sender.getName()+CommonColors.ERROR+" failed security check (revoking permissions from "+targetPlayer.getName()+").");
					return Error.NO_PERMISSION.displayChat(sender);
				}
