...
/**
 * A collection of utilities that are used throughout multiple components and
 * plugins.
 *
...
public final class Utils {
	...
	/**
	 * Gets a view of all currently logged in admins (players with permission
	 * "core.admin").
	 *
	...
	/**
	 * Sends an error/warning message to all online admins (players with permission
	 * "core.admin").
	 *
	 * @param message the message to send
	 */
	...
	public static void notifyAdmins(Level level, String message) {
		// Prepare the message
		ChatColor prefixColor = (level == Level.SEVERE || level == Level.WARNING) ? CommonColors.ERROR.getColor()
				: CommonColors.MESSAGE.getColor();
	...
	/**
	 * Gets a string with the names of all online admins (players with permission
	 * "core.admin"), seperated by commas.
	 *
	...
	public static String listOnlineAdmins() {
		...
		getOnlineAdmins().forEach(player -> {
			if (!player.hasPermission("core.invisible"))
				onlineAdmins.append(player.getDisplayName()).append(CommonColors.INFO + ", ");
		});

		// If no online admins, show Phoenix, otherwise remove the extra comma at the
		// end
		if (onlineAdmins.length() < 3)
			onlineAdmins
					.append(CoreConfig.enableWolfiaFeatures ? ChatColor.DARK_RED + "P" + ChatColor.DARK_GRAY + "hoenix"
							: "None");
		else
	 ...
	 * @param checkOfflinePlayer true to get offline players (otherwise only online
	...
	public static OfflinePlayer getPlayer(String name, boolean checkOfflinePlayer) {
		...
		if (player == null && checkOfflinePlayer && CoreConfig.searchOfflineNames) {
			// Look through every player that has ever joined, and see if they're an exact
			// match
	...
	/**
	 * Gets a player object for the given username or display name. Offline players
	...
	/**
	 * Checks if this server is running the latest version of Minecraft (at the time
	 * this plugin was released).
	 *
	 * @return true if this server is running the Minecraft version that this plugin
	 *         was designed for
	 * @deprecated use version-agnostic code wherever possible - this will always
	 *             return false
	 */
	@Deprecated
	...
	/**
	 * Sends an action bar message to a {@link Player}. Use this to send
	 * non-critical command feedback and status messages to the player.
	 *
	...
	public static void sendActionBar(Player target, String message) {
		...
		// If enabled, use Spigot Chat Component API to send action bar message
		if (CoreConfig.useActionBarMessages)
	...
	/**
	 * Sends an action bar message to a {@link CommandSender}. Use this to send
	 * non-critical command feedback and status messages to the player. If the
	 * target is not a player, a normal chat message will be sent instead.
	 *
	...
	public static void sendActionBar(CommandSender target, String message) {
		...
		if (target instanceof Player)
			sendActionBar((Player) target, message);

		// If it's not a player, send a normal message
		else
			target.sendMessage(ChatColor.stripColor(message));
	}
	...
	public static void sendRawJson(Player target, String rawJsonMsg) {
		// Need to find a proper way to do this through Spigot API

		// Otherwise, use vanilla command to send message
		Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "minecraft:tellraw " + target.getName() + " " + rawJsonMsg);
	}
